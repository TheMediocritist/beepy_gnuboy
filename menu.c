...
static void menu_paint(void) {
	...
	for (y = 0; y < ezm.h; ++y) {
		...
		for (x = 0; x < ezm.w; ++x) {
			if (y == ezm.vissel) {
				if (x == 0) {
					font_blit(screen, (x + 1) * FONTW, y * FONTH, '0', 0); // Draw the '>' character
				}
				else if (x == 1) {
					font_blit(screen, (x + 1) * FONTW, y * FONTH, '_', 0); // Draw the '_' character
				}
				else {
					font_blit(screen, (x + 3) * FONTW, y * FONTH, x >= l ? ' ' : ezm.vislines[y][x], 0);
				}
			} else {
